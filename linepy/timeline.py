...
class Timeline(Channel):
    ...
    def likePost(self, mid, postId, likeType=1001):
        ...
        data = {'likeType': likeType, 'activityExternalId': postId, 'actorId': mid, 'sharable': True}
        data = json.dumps(data)
        r = self.server.postContent(url, data=data, headers=self.server.timelineHeaders)
        return r.json()

    @loggedIn
    def followUser(self, mid):
        params = {'userId': mid}
        url = self.server.urlEncode(self.server.LINE_TIMELINE_API, '/v1/follow/followUser/'+mid, params)        
        r = self.server.putContent(url, headers=self.server.timelineHeaders)
        return r.json()
    ...
    def reportPost(self, mid, postId):
        ...
        data = {'postId': postId, 'reason': 'R0014'}
        r = self.server.postContent(url, data=data, headers=self.server.timelineHeaders)
        return r.json()
